...
if page == "Dashboard":
    ...
    if recipes.empty:
        ...
        default_status = "Finished" if int(r["cure_days"] or 0) == 0 else "Curing"
        cure_status = c3.selectbox("Manual Cure Status", cure_status_options, index=cure_status_options.index(default_status))

        cure_date = made_date + timedelta(days=int(r["cure_days"] or 0))
        qty = st.number_input("Quantity Made", min_value=1, value=int(r["bars_made"] or 1))
        ...
        if st.button("Save Batch and Add to Finished Goods"):
            ...
            try:
                ...
                goods_data.pop("cure_status", None)
                insert("finished_goods", goods_data)
            st.rerun()
    ...
    if goods.empty:
        ...
        if "cure_status" not in goods.columns:
            goods["cure_status"] = "Curing"
        goods["total_value"] = goods["quantity_on_hand"].astype(float) * goods["cost_per_item"].astype(float)

        if not batches.empty and "batch_id" in goods.columns:
            batch_cols = [c for c in ["id", "batch_number", "date_made", "cure_date"] if c in batches.columns]
            if batch_cols:
                goods = goods.merge(batches[batch_cols], left_on="batch_id", right_on="id", how="left", suffixes=("", "_batch"))

        search_term = st.text_input("Search finished goods", placeholder="Search product, type, batch, status, or notes...")
        status_filter = st.selectbox("Filter by Cure Status", ["All"] + cure_status_options)
        ...
        if display_goods.empty:
            ...
            header = st.columns([2, 1.1, 1, 1, 1, 1, 1.2])
            header[0].markdown("**Product**")
            header[1].markdown("**Batch**")
            header[2].markdown("**Qty**")
            header[3].markdown("**Ready Date**")
            header[4].markdown("**Status**")
            header[5].markdown("**Value**")
            header[6].markdown("**Update**")
            ...
            for _, row in display_goods.sort_values(["cure_status", "product_name"]).iterrows():
                ...
                cols[3].write(row.get("cure_date") or "—")
                new_status = cols[4].selectbox("Status", cure_status_options, index=cure_status_options.index(current_status), key=f"goods_status_{gid}", label_visibility="collapsed")
                cols[5].write(f"${float(row.get('total_value') or 0):.2f}")

                if cols[6].button("Save Status", key=f"save_goods_status_{gid}"):
                    try:
                        update_row("finished_goods", gid, {"cure_status": new_status})
                        st.rerun()
                    except Exception as e:
                        st.error("The cure_status column is missing in Supabase. Run the v1.2.7 SQL update below, then try again.")
                        st.code(str(e))
